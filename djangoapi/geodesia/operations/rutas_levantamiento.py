...
class RutasLevantamientoCrud:
    ...
    def selectAll(self, d=None):
        try:
            objs = RutasLevantamiento.objects.all().order_by("id")

            data = []

            for obj in objs:
                #usa model_to_dict(obj) para copiar automáticamente todos los campos del modelo.
                #FUNCIONA CON EL from django.forms.models import model_to_dict
                item = model_to_dict(obj)
                item["geom"] = obj.geom.wkt if obj.geom else None
                data.append(item)

                #SINO INSERTARIAMOS MANUALMENTE CADA CAMPO
                #data.append({
                #    "id": obj.id,
                #    "codigo_ruta": obj.codigo_ruta,
                #    "equipo_usado": obj.equipo_usado,
                #    "operador": obj.operador,
                #    "fecha_toma": str(obj.fecha_toma),
                #    "observaciones": obj.observaciones,
                #    "longitud": obj.longitud,
                #    "geom": obj.geom.wkt if obj.geom else None,
                #})
